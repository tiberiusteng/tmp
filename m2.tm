...
\h1[Text Markup Processor 簡介]

Text Markup Processor (以下簡稱 tmp) 是個以 \url(Python)[http://www.python.
org/] 撰寫的轉譯器架構。
...
比如說用 \url(vim)[http://www.vim.org/] 配上語法彩色檔)，進行跳脫就會變得十分
...
標題 (\c[\\title])，並且打開自動產生目錄 (\c[\\buildtoc])、打開分頁模式
產生頁面連結 (\c[\\paging]) 等等功能；最後再以 \c[\\body] 組織全篇頁面內容。
...
\ul[
\li[\c[\\buildtoc]: 自動產生目錄 (→ 請參閱「標題」)]
\li[\c[\\paging]: 分頁模式 (→ 請參閱「頁數編碼」)]
]
...
如果您以 \c[\\paging] 打開分頁模式的話，就能在內容之中以 \c[\\page] 標籤標示
目前所在的頁次。頁次會出現在左邊的黃色頁邊範圍之內，同時也會自動在頁首的
 \b[Page Link] 區域內自動產生連結，讓您能夠直接跳躍到指定的頁面。

如果在同一個 \c[\\page] 標籤內以逗號列出許多個頁碼的話，即可連續列出這些頁碼，
並且略去這些頁碼之間的頁面分隔線 (作者在遇到橫跨多頁的表格時，不想把表格切斷，
卻又想保留中途頁碼連結時常用)。

Page Link 位於頁首的 Metadata 資料區之內，您可點選黃色的 Show/Hide
 Metadata 標籤切換本區域顯示與否。

如果沒有打開分頁模式卻用了 \c[\\page] 標籤的話，就會產生錯誤訊息；這是為了避免
頁碼標示對版面造成影響所作的設計。
...
連結功能由 \c[\\url] 標籤提供。它的作用範圍即是要連結的網址，如果您希望自訂
連結顯示文字的話，只要以參數的形式提供即可。

\boxHead[範例]
\code[[
\\url[http://www.google.com/], \\url(Python 程式語言)[http://www.python.org/]
]]
...
緊接在 \c[\\table] 標籤之後，您可接著使用 \c[\\caption] 標籤標記代表整個表格的
...
這是段落\\fn[很長的註腳很長的註腳很長的註腳很長的註腳很長的註腳
很長的註腳很長的註腳]這是段落這是段落這是段落這是段落這是段落
這是段落這是段落
...
\h3[TODO: 自動索引]
 ...
 \c[\\buildindex] 啟動自動索引建構模式，以 \c[\\kw] (keyword) 指定文內需要進行
索引的關鍵字。
